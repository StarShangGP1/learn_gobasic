...
func TestSuggestion_GetSuggestion(t *testing.T) {
	suggest := GetFatRateSuggestion()
	tests := []Person{
		{
			sex:     "男",
			age:     35,
	...
	if got := suggest.GetSuggestion(&tests[0]); got != "肥胖" {
		t.Fail()
	}

}

func TestSuggestion_GetAllSuggestion(t *testing.T) {
	suggest := GetFatRateSuggestion()
	type args struct {
		person *Person
	}
